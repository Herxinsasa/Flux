...
keywords: [日志分析, 排查总结, 问题总结, 问题总结报告, 故障复盘, 根因, 知识库, 问题沉淀, 归纳, 验证步骤, 技术问题, 故障报告]
---
...
- **七段模板正文必须完整出现在对话回复中**，不得仅通过 `write_file` 写入 `.md` 文件代替。
- 除非用户**明确指定路径并要求落盘**，否则**禁止**用 `write_file` 保存问题总结/复盘条目。
- 不要声称文件已导出；提示用户可使用对话下方的 **「导出报告」** 保存 Markdown。
