...
export async function runAiAction(request: AiActionRequest): Promise<AiActionRunResult> {
  ...
  if (activeActions.has(key)) throw new Error('该文档已有 AI 操作正在执行')
  ...
  if (!providerId) throw new Error('请先在设置中配置并启用 AI 提供商')
  const provider = store.get('providers').find((item) => item.id === providerId)
  if (!provider) throw new Error('当前 AI 提供商不可用，请检查设置')
  ...
  try {
    ...
    for await (const event of client.chat({
      ...
      if (active.cancelled) throw new Error('AI 操作已取消')
      if (event.type === 'text_delta') rawText += event.text
      if (event.type === 'error') throw new Error(event.message)
    }
    if (active.cancelled) throw new Error(timedOut ? 'AI 操作超时，请重试' : 'AI 操作已取消')
    if (!rawText.trim()) throw new Error('AI 未返回可用内容，请重试')
    return {
      requestId: request.requestId,
