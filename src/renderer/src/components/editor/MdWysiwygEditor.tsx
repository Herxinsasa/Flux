...
import { $inputRule, callCommand, replaceAll } from '@milkdown/kit/utils'
import {
  AlignCenter,
  AlignLeft,
  AlignRight,
...
import {
  frontmatterNode,
  registerFrontmatterParsing,
  registerFrontmatterStringify,
...
import {
  ...
  registerWysiwygLineBreakStringify,
} from './wysiwygLineBreaks'
import { isTableCellSelection, placeTableCaretFromPointer } from './wysiwygTableInteractions'
import { createWysiwygImageView, type WysiwygImagePickRequest } from './wysiwygImageView'
import { isSupportedImageFile, saveMarkdownImages } from '../../utils/imageAttachment'
import {
  captureConnectedEditorScrollPosition,
  editorScrollRatio,
  editorScrollTopForRatio,
  type EditorScrollPosition,
} from '../../utils/editorScrollPosition'
import {
...
const strictGfm = gfm.filter((plugin) => plugin !== strikethroughInputRule)
const strictStrikethroughInputRule = $inputRule((ctx) =>
  markRule(/(?<!~)(~~)(?![~\s])(.+?\S)\1(?!~)$/, strikethroughSchema.type(ctx)),
)
...
function restoreTrailingHardBreak(view: EditorView, markdown: string): void {
  if (!hasTrailingMarkdownHardBreak(markdown)) return
  const lastBlock = view.state.doc.lastChild
  const hardBreak = view.state.schema.nodes.hardbreak
  if (!lastBlock?.isTextblock || !hardBreak || lastBlock.lastChild?.type === hardBreak) return
  const transaction = view.state.tr.setSelection(TextSelection.atEnd(view.state.doc))
  view.dispatch(transaction.replaceSelectionWith(hardBreak.create()))
}

function scrollEditorPositionIntoView(
  view: EditorView,
...
function MdWysiwygEditorInner({
  ...
  const reviewDocument = useReviewStore((state) =>
    currentFile ? state.documents[normalizeDocumentPath(currentFile)] : undefined,
  )
  ...
  const insertImageFiles = useCallback(
    async (files: File[], target?: { position?: number; replace?: boolean }) => {
      ...
      if (!view || !imageType) return true
      const imageNodes = result.images.map((image) =>
        imageType.create({
          src: image.markdownPath,
          alt: image.alt,
          title: '',
        }),
      )
      ...
      if (target?.replace && targetPosition != null) {
        const existing = view.state.doc.nodeAt(targetPosition)
        if (existing?.type === imageType) {
          transaction = transaction.setNodeMarkup(targetPosition, imageType, imageNodes[0].attrs)
          let insertAt = targetPosition + existing.nodeSize
          for (const imageNode of imageNodes.slice(1)) {
            transaction = transaction.insert(
              insertAt,
              Fragment.fromArray([view.state.schema.text(' '), imageNode]),
            )
            insertAt += imageNode.nodeSize + 1
          }
        }
      } else {
        if (targetPosition == null) transaction = transaction.deleteSelection()
        const insertAt = targetPosition ?? transaction.selection.from
        ...
        transaction = transaction.insert(
          insertAt,
          new Slice(Fragment.fromArray(nodes), 0, 0).content,
        )
      }
  ...
  const requestImagePick = useCallback((request: WysiwygImagePickRequest) => {
    if (isReadOnlyRef.current) return
    pendingImagePositionRef.current = request.position
  ...
  useEffect(() => {
    ...
    let releaseRestoreFrame: number | null = null
    let restoringScroll = true
    let lastScrollPosition: EditorScrollPosition | null = null
    let lastMarkdown = normalizeWysiwygMarkdown(useEditorStore.getState().content)
    ...
    const captureScrollPosition = () => {
      lastScrollPosition = captureConnectedEditorScrollPosition(
        scrollContainer,
        lastScrollPosition,
      )
      return lastScrollPosition
    }
    const writeSurfaceState = (persistSharedScroll: boolean) => {
      const view = viewRef.current
      if (!view) return
      const scrollPosition = captureScrollPosition()
      const scrollTop =
        scrollPosition?.scrollTop ?? wysiwygSurfaceStates.get(fileKey)?.scrollTop ?? 0
      wysiwygSurfaceStates.set(fileKey, {
        selection: { from: view.state.selection.from, to: view.state.selection.to },
        scrollTop,
      })
      if (persistSharedScroll && scrollPosition && !restoringScroll) {
        useEditorStore
          .getState()
          .setDocumentScrollTop(scrollTop, scrollPosition.scrollRatio)
      }
    }
    const saveSurfaceState = () => writeSurfaceState(true)
    const scheduleSurfaceStateSave = () => {
    ...
    const restoreSurfaceState = () => {
      ...
      let attempts = 0
      let previousScrollMaximum = -1
      let stableLayoutFrames = 0
      const restoreWhenLayoutReady = () => {
        restoreFrame = window.requestAnimationFrame(() => {
          ...
          const session = editorState.activeDocumentPath
            ? editorState.documentSessions[editorState.activeDocumentPath]
            : undefined
          const ratio = session?.scrollRatio
          const scrollMaximum = Math.max(
            0,
            currentScrollContainer.scrollHeight - currentScrollContainer.clientHeight,
          )

          if (ratio != null && ratio > 0) {
            stableLayoutFrames =
              scrollMaximum > 0 && Math.abs(scrollMaximum - previousScrollMaximum) < 1
                ? stableLayoutFrames + 1
                : 0
            previousScrollMaximum = scrollMaximum
            if ((scrollMaximum === 0 || stableLayoutFrames < 2) && attempts < 12) {
              attempts += 1
              restoreWhenLayoutReady()
              return
            }
          }
          ...
          currentScrollContainer.scrollTop =
            ratio != null
              ? editorScrollTopForRatio(currentScrollContainer, ratio)
              : saved?.scrollTop ?? 0
          lastScrollPosition = {
            scrollTop: currentScrollContainer.scrollTop,
            scrollRatio: editorScrollRatio(currentScrollContainer),
          }
          releaseRestoreFrame = window.requestAnimationFrame(() => {
            restoringScroll = false
            releaseRestoreFrame = null
          })
        })
      }
      restoreWhenLayoutReady()
    ...
    const editor = Editor.make()
      .config((ctx) => {
        ...
        ctx.get(listenerCtx).markdownUpdated((_ctx, markdown) => {
          const lastBlock = viewRef.current?.state.doc.lastChild
          const normalizedMarkdown = normalizeWysiwygMarkdown(
            preserveTrailingMarkdownHardBreak(
              markdown,
              lastBlock?.lastChild?.type.name === 'hardbreak',
            ),
          )
          if (disposed || normalizedMarkdown === lastMarkdown) return
          if (hydratingMarkdown) return
          if (!userEditRef.current && !useEditorStore.getState().isDirty) {
            lastMarkdown = normalizedMarkdown
            return
          }
          lastMarkdown = normalizedMarkdown
          onCommitRef.current(normalizedMarkdown)
        })
      ...
      .use(tocRemarkPlugin)
      .use(strictGfm)
      .use(strictStrikethroughInputRule)
      ...
      .use(
        createWysiwygImageView({
          baseFilePath: currentFile,
          onPick: requestImagePick,
    ...
    const destroyEditor = (): Promise<void> => {
      if (!createStarted) return Promise.resolve()
      if (!destroyPromise) {
        destroyPromise = Promise.resolve()
          .then(() => editor.destroy())
          .catch((error: unknown) => {
            console.error(`[MdWysiwygEditor] editor.destroy 失败 fileKey=${fileKey}`, error)
          })
      }
    ...
    const createEditor = async (): Promise<void> => {
      ...
      try {
        ...
        const latestMarkdown = normalizeWysiwygMarkdown(useEditorStore.getState().content)
        try {
          viewRef.current = editor.ctx.get(editorViewCtx)
          viewRef.current.dom.spellcheck = false
        } catch {
          viewRef.current = null
        }
        // 文件读取可能先于 Milkdown 初始化完成；补齐订阅建立前错过的正文快照。
        if (latestMarkdown !== lastMarkdown) {
          lastMarkdown = latestMarkdown
          editor.action(replaceAll(latestMarkdown, true))
        }
        if (viewRef.current) restoreTrailingHardBreak(viewRef.current, latestMarkdown)
        hydratingMarkdown = false
        // view 就绪后触发跳转 effect 补执行（初始化窗口期内的双击/大纲点击不再丢失）
        if (viewRef.current) {
          setViewReady(true)
          scrollContainer = root.closest<HTMLElement>('.flux-scroll')
          restoreSurfaceState()
          ...
          scrollContainer?.addEventListener('scroll', scheduleSurfaceStateSave, { passive: true })
        }
        unsubscribeStore = useEditorStore.subscribe((state, previousState) => {
          ...
          const hydratedMarkdown = normalizeWysiwygMarkdown(state.content)
          if (hydratedMarkdown === lastMarkdown) return
          lastMarkdown = hydratedMarkdown
          editor.action(replaceAll(hydratedMarkdown, true))
        })
        // 批注高亮：批注列表变化时刷新装饰
        ...
        const refreshDecorations = (force = false) => {
          ...
          const document =
            useReviewStore.getState().documents[normalizeDocumentPath(currentFile ?? '')]
        ...
        console.error(`[MdWysiwygEditor] editor.create 失败 fileKey=${fileKey}`, error)
        setViewReady(false)
        useEditorStore.getState().setMarkdownEditSurface('source')
        await destroyEditor()
      }
    }

    const queueCreate = lifecycleQueueRef.current
      .catch((error: unknown) => {
        console.error(`[MdWysiwygEditor] lifecycle queue 失败 fileKey=${fileKey}`, error)
      })
      .then(createEditor)
    lifecycleQueueRef.current = queueCreate.catch((error: unknown) => {
      console.error(`[MdWysiwygEditor] lifecycle task 失败 fileKey=${fileKey}`, error)
    })

    return () => {
      writeSurfaceState(false)
      if (scrollSaveFrame != null) window.cancelAnimationFrame(scrollSaveFrame)
      if (restoreFrame != null) window.cancelAnimationFrame(restoreFrame)
      if (releaseRestoreFrame != null) window.cancelAnimationFrame(releaseRestoreFrame)
      ...
      scrollContainer?.removeEventListener('scroll', scheduleSurfaceStateSave)
      disposed = true
      unsubscribeStore?.()
      ...
      setViewReady(false)
      const queueDestroy = lifecycleQueueRef.current
        .catch((error: unknown) => {
          console.error(`[MdWysiwygEditor] lifecycle queue 失败 fileKey=${fileKey}`, error)
        })
        .then(destroyEditor)
      lifecycleQueueRef.current = queueDestroy.catch((error: unknown) => {
        console.error(`[MdWysiwygEditor] lifecycle destroy 失败 fileKey=${fileKey}`, error)
      })
    }
  }, [fileKey])
  ...
  useEffect(
    () =>
      listenForMarkdownCommands((command) => {
    ...
    (
      ...
      if (command === 'add-row') editor.action(callCommand(addRowAfterCommand.key))
        ...
        if (!isTableCellSelection(view.state.selection)) {
          ...
          editor.action(
            callCommand(selectColCommand.key, {
              index: columnIndex,
  ...
  useEffect(() => {
    ...
    view.state.doc.descendants((node, position) => {
      ...
      if (Number(node.attrs.level) !== outlineTarget.level || nodeText !== outlineTarget.text)
        return
    ...
    const targetDom =
      rawDom instanceof Element ? (rawDom as HTMLElement) : (rawDom?.parentElement ?? null)
  ...
  const handleContextMenu = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (
      position &&
      (selection.empty || position.pos < selection.from || position.pos > selection.to)
    ) {
      view.dispatch(
    ...
    const markdown = normalizeWysiwygMarkdown(useEditorStore.getState().content)
    ...
    if (!selection.empty && selectedText) {
      ...
      const markedDoc = view.state.tr
        ...
        .insertText(startMarker, selection.from).doc
      const markedMarkdown = normalizeWysiwygMarkdown(editor.ctx.get(serializerCtx)(markedDoc))
  ...
  const handleImageDrop = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (files.length === 0) return
      event.preventDefault()
      const position = viewRef.current?.posAtCoords({
        left: event.clientX,
        top: event.clientY,
      })?.pos
  ...
  return (
    <div
      ...
      data-zoom-below-one={contentZoom < 1 ? 'true' : undefined}
    ...
    >
      <input
        ...
        onChange={(event) => {
          ...
          if (files.length > 0 && position != null) {
            void insertImageFiles(files, { position, replace: true })
          }
        }}
      />
      ...
      {tableTools && (
        <div
          className="flux-table-toolbar"
          contentEditable={false}
          style={{ left: tableTools.left, top: tableTools.top }}
        ...
        >
          <button
            ...
            onClick={() => setTableSizeOpen((value) => !value)}
          >
            <TableProperties size={16} />
          </button>
          <button type="button" title="左对齐" onClick={() => runTableCommand('left')}>
            <AlignLeft size={16} />
          </button>
          <button type="button" title="居中" onClick={() => runTableCommand('center')}>
            <AlignCenter size={16} />
          </button>
          <button type="button" title="右对齐" onClick={() => runTableCommand('right')}>
            <AlignRight size={16} />
          </button>
          {tableSizeOpen && (
            <div className="flux-table-size-popover">
              <div>
                <Rows3 size={15} />
                <span>行</span>
                <button
                  type="button"
                  title="减少一行"
                  disabled={tableTools.rows <= 2}
                  onClick={() => runTableCommand('remove-row')}
                >
                  <Minus size={14} />
                </button>
                <output>{tableTools.rows}</output>
                <button type="button" title="增加一行" onClick={() => runTableCommand('add-row')}>
                  <Plus size={14} />
                </button>
              </div>
              <div>
                ...
                <button
                  type="button"
                  title="减少一列"
                  disabled={tableTools.columns <= 1}
                  onClick={() => runTableCommand('remove-column')}
                >
                  <Minus size={14} />
                </button>
                <output>{tableTools.columns}</output>
                <button
                  type="button"
                  title="增加一列"
                  onClick={() => runTableCommand('add-column')}
                >
                  <Plus size={14} />
                </button>
              </div>
        ...
        <WysiwygReviewComposer
          ...
          onSave={async (body) => {
            ...
            const saved = await useReviewStore
              .getState()
              .addComment(currentFile, content, reviewComposer.anchor, body)
