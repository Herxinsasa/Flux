...
- **单实例文档接管** — 通过「打开方式」再次启动 Flux 时，由现有窗口接收文档；保留当前工作区，并将外部文档加入工作集
...
- **本地图片** — 支持相对路径、Windows 绝对路径及本地 SVG；可粘贴或拖入多张图片，并自动生成标准 Markdown 相对路径
...
- **受控文件工具** — Agent 读取和搜索仅限当前工作区、已打开文件及显式附件路径；写入先生成差异预览，由用户确认后落盘
...
### 1. 日志分析与故障排查
- 适用对象：运维、后端开发、测试、SRE
- 操作方式：打开日志文件或日志目录，结合 AI 对话做错误归因、时间线梳理、风险分级。
- 常见产出：问题总结、根因说明、修复建议、可归档 Markdown 报告。

### 2. Markdown 文档编辑与知识沉淀
- 适用对象：技术写作者、研发团队、项目负责人
- 操作方式：在源码编辑、实时预览、WYSIWYG 模式间切换，快速整理设计文档、复盘、知识库条目。
- 常见产出：README、技术方案、故障复盘、周报与交付说明。

### 3. 多文件上下文问答与代码解释
- 适用对象：需要跨文件理解系统的开发者
- 操作方式：通过 `@文件` 引入关键上下文，必要时使用 `/Skill` 约束分析流程。
- 常见产出：模块职责说明、调用链分析、改动建议与风险点清单。

### 4. AI 辅助报告导出与团队协作
- 适用对象：需要对外同步分析结果的团队
- 操作方式：在对话结束后一键导出 Markdown 报告，直接进入评审、归档或分享流程。
- 常见产出：分析报告、问题总结报告、会议前阅读材料。

### 5. 本地多 Provider 模型调试
- 适用对象：需要切换不同大模型能力的用户
- 操作方式：在设置页配置不同 Provider 与模型，测试连通后用于同一工作区任务对比。
- 常见产出：模型效果对比结论、稳定配置模板。
